  ...
  switch (i) {
    ...
    case 1: // mix
      { float r = 0.001f * v;
        echo_left.setMix(r);
        echo_right.setMix(r);
        ss << "Mix: " << fixed << setprecision(1) << (0.1f*v) << "%";
      }
      break;
    case 2: // feedback
      { float f = 0.001f * v;
        ...
        ss << "Feedback: " << fixed << setprecision(1) << (0.1f*v) << "%";
      }
      break;
    case 3: // gain
      gain = 0.002f * v;
      ss << "Gain: " << fixed << setprecision(1) << (0.2f*v) << "%";
  }
...
int OnWrite(const void *vin, void *vout, unsigned long frames,
  ...
  if (et.stereo) {
    for (unsigned i=0; i < frames; ++i) {
      float L = IMAX * et.samples[2*et.index],
            R = IMAX * et.samples[2*et.index+1];
      L = et.echo_left(L);
      R = et.echo_right(R);
      out[2*i] = et.gain * L;
      out[2*i+1] = et.gain * R;
    ...
    for (unsigned i=0; i < frames; ++i) {
      float y = IMAX * et.samples[et.index];
      y = et.echo_left(y);
      out[i] = et.gain * y;
      et.index = (et.index+1) % et.count;
    }
